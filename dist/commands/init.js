...
async function runInit(options = {}) {
    ...
    await (0, fs_1.ensureDir)(path_1.default.join(root, "tasks"));
    await (0, fs_1.ensureDir)(path_1.default.join(root, ".github", "workflows"));
    // Core instruction files
    await createIfMissing(path_1.default.join(root, ".aicontext", "AI_WORKFLOW_TASKS.md"), AI_WORKFLOW_TASKS_CONTENT, summary);
    await createIfMissing(path_1.default.join(root, ".aicontext", "AI_MCP_INSTRUCTIONS.md"), AI_MCP_INSTRUCTIONS_CONTENT, summary);
    await createIfMissing(path_1.default.join(root, ".aicontext", "AI_GITMCP_INTEGRATION.md"), AI_GITMCP_INTEGRATION_CONTENT, summary);
    await createIfMissing(path_1.default.join(root, ".aicontext", "MCP_SERVERS.md"), MCP_SERVERS_CONTENT, summary);
    await createIfMissing(path_1.default.join(root, ".aicontext", "CONTEXT_COMPACTION.md"), CONTEXT_COMPACTION_CONTENT, summary);
    await createIfMissing(path_1.default.join(root, ".aicontext", "AI_README.md"), AI_README_CONTENT, summary);
    ...
    // Sample task
    const tasksDirEntries = await promises_1.default.readdir(path_1.default.join(root, "tasks"));
    const hasTasks = tasksDirEntries.some((name) => /^T-[^/]+\.md$/i.test(name));
    if (!hasTasks && sampleEnabled) {
        await createIfMissing(path_1.default.join(root, "tasks", "T-0001-sample-task.md"), buildSampleTask(), summary);
    }
    else if (!sampleEnabled) {
        summary.skipped.push("Sample task (--no-sample)");
    }
    // GitHub workflow
    await createIfMissing(path_1.default.join(root, ".github", "workflows", "jalco-repoAI-auto-sync.yml"), WORKFLOW_CONTENT, summary);
    // Summary logging
    console.log("jalco-repoAI init finished.");
    const formatItem = (item) => (path_1.default.isAbsolute(item) ? path_1.default.relative(root, item) : item);
